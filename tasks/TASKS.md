...
6) + Define Agent Principles and Orchestrator
   Action: Define the agent's high-level principles, establish key terms ("Orchestrator", "Agent"), and create the core orchestrator script for LLM interaction.
   Acceptance: The documentation (`docs/AGENT_PRINCIPLES.md`) exists defining principles and terminology. A script (`scripts/run_local_agent.py`) exists that allows interaction with an agent. Both components are integrated to define the agent's core functionality.

7) + Tests specification
...
9) - The plans for all tasks must be updated
...
10) - New child projects structure
...
13) ? First child project -> Libraries
...
18) ? Create a way to easily launch MCP servers
...
20) - explore other ways to launch agents locally
