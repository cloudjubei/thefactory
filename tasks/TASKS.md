...
6) + Agent Orchestration and Principles
   Action: Create the specification that defines the agent's high-level principles, establishes key terms, and create a script that functions as the Agent's Orchestrator. The agent uses a tool-based architecture.
   Acceptance: The documentation `docs/AGENT_PRINCIPLES.md` exists and a script `scripts/run_local_agent.py` exists that allows interaction with an agent.
