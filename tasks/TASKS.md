...
9) - Should the tasks and plan format change?
...
14) - Create 'move_task' tool
   Action: Create a tool that allows an agent to reorder tasks in tasks/TASKS.md. The tool should be able to move a task from a source position to a destination position, automatically re-numbering all affected tasks.
   Acceptance:
     - A new tool, `move_task`, is available to the agent.
     - The tool correctly modifies `tasks/TASKS.md` to reflect the new order.
     - The tool is documented in `docs/TOOL_ARCHITECTURE.md`.
     - The tool has corresponding tests.
   Notes: This task was created by refining and splitting the original Task 9.
...
17) ? Create orchestration so that many different agents can be running on different tasks at once.
...
20) ? Create a way to easily launch MCP servers
...
22) - explore other ways to launch agents locally
