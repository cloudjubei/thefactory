...
class AgentTools:
    ...
    def submit_for_review(self, task_id: int, task_title: str):
        ...
        # Using git_manager to handle the multi-step git process
...
class UnifiedEngine:
    ...
    def _build_prompt(self, context: dict) -> list:
        ...
        # ** THE FIX IS HERE: The prompt is now much simpler and safer **
        system_prompt = f"""
...
- `submit_for_review(task_id, task_title)`: Adds, commits, and creates a pull request for all changes.
- `ask_question(question_text)`: Halts all work and asks the user a question.
- `finish(reason)`: Stops the current work cycle.

**YOUR WORKFLOW IS MANDATORY:**
1.  **Analyze** the context and `TASKS.md` to identify the next eligible pending task.
2.  **Formulate a plan** to complete this single task.
3.  **Generate `tool_calls` to:**
    a.  **`write_file`** for all necessary code and documentation changes.
    b.  **`write_file`** to update the task's status from `-` to `~` in `TASKS.md`.
    c.  **`submit_for_review`** with the correct `task_id` and `task_title`.
    d.  **`finish`** to end your work cycle.
...
"""
    ...
    # ... _make_api_call and _parse_response are unchanged ...
...
class Agent:
    # ** THE FIX IS HERE: `allow_dangerous_tools` is removed **
    ...
    def _execute_tool_calls(self, tools_instance: AgentTools, tool_calls: list) -> bool:
        # ** THE FIX IS HERE: The security check is now implicit as no dangerous tools exist **
...
if __name__ == "__main__":
    ...
    # ** THE FIX IS HERE: The dangerous flag is removed from the parser **
